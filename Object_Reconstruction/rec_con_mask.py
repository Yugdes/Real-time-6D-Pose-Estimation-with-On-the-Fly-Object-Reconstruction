...
with open("last_object_name.txt", "w") as f:
    f.write(object_name)

base_dir = os.path.join('input', object_name)
sub_dirs = ['depth', 'rgb']
...
try:
    ...
    pipeline.stop()
    cv2.destroyAllWindows()
    # ✅ Save number of frames captured
    with open(os.path.join(base_dir, 'frame_count.txt'), 'w') as f:
        f.write(str(frame_count)) 
    print(f"✅ Saved frame count: {frame_count}")

source_folder = os.path.join('input', object_name, 'rgb')
destination_folder = os.path.join('input', object_name, 'JPEGImages', 'video1')
...
image_path = os.path.join('input', object_name, 'JPEGImages', 'video1', '00000001.jpg')
...
annotations_dir = os.path.join('input', object_name, 'Annotations', 'video1')
os.makedirs(annotations_dir, exist_ok=True)

# Save the mask image
cv2.imwrite(os.path.join(annotations_dir, '00000000.png'), mask)
