...
def create_mask_from_image(image):
    """
    Allows user to draw a binary mask over a given image.
    Returns the path to the saved mask file.
    """
    mask = np.zeros(image.shape[:2], dtype=np.uint8)
    drawing = [False]  # mutable flag
    display = image.copy()
    ...
    window_name = "Draw Mask - Press ENTER when done"
    ...
    temp_mask_path = os.path.join(tempfile.gettempdir(), "mask.png")
    cv2.imwrite(temp_mask_path, mask)
    return temp_mask_path

if __name__ == "__main__":
    mask_file_path = create_mask_from_image()
